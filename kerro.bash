...
if [ $tunti -ge 24 ]
then vrkaika="yötä, mene nukkumaan on jo sen aika"
fi
...
lause="Hyvää "$vrkaika" "$kayttaja". Tänään on "$viikonpaiva" "$pv" päivä "$kuukausi" "$vuosi". Kello on "$tunti" "$minuutti

echo $lause>/tmp/ttv && iconv -f UTF-8 -t ISO8859-1 /tmp/ttv > /tmp/ttv2 && festival --language finnish --tts /tmp/ttv2 >/dev/null
